...
public class ContactsManager
{
    ...
    public void LoadContactsFromFile(string fileName)
    {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                if (parts.Length > 0)
                {
                    if (parts[0] == "CHANGE")
                    {
                        ...
                        int contactId = int.Parse(parts[1]);
                        var fieldName = parts[2];
                        var oldValue = parts[3];
                        var newValue = parts[4];
                        var timestamp = DateTime.Parse(parts[5]);

                        currentContact = Contacts.FirstOrDefault(c => c.ID == contactId);

                        if (currentContact != null)
                        {
                            currentContact.AddChange(fieldName, oldValue, newValue);
                        }
                    }
                    else
                    {
                        ...
                        currentContact = new Contact(id, name, phoneNumber, email, address);
        ...
        Console.WriteLine("Контакти та історію змін завантажено з файлу.");
    }
    ...
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case "6":
                    Console.Write("Введіть ім'я файлу для завантаження контактів: ");
